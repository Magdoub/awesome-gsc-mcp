 ...
 *
 * Tokens are persisted to ~/.awesome-gsc-mcp/token.json so the user only
...
const TOKEN_DIR = join(homedir(), '.awesome-gsc-mcp');
const TOKEN_PATH = join(TOKEN_DIR, 'token.json');
const SCOPES: readonly string[] = ['https://www.googleapis.com/auth/webmasters'];
